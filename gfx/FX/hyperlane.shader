...
VertexShader =
{
	...
	[[
		VS_QUAD_OUTPUT main( const VS_INPUT v )
		{
			...
			// --- MODDED LINE: compute alpha from position ---
			// Replaces the vanilla "Out.PrimaryColor = v.PrimaryColor;"
			Out.PrimaryColor = float4(
				1.0, 1.0, 1.0,
				clamp( 0.23f 
					   - abs( v.Position.x / 4700.f ) 
					   - abs( v.Position.z / 4700.f ),
					   0.06f, 0.23f )
			);
